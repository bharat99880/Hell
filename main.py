...
async def stop_raid(event):
    if not raid_targets:
        await event.reply("No active raids running.")
        return
    raid_targets.clear()
    await event.reply("All raids stopped successfully.")
...
# ────────────── AUTO GRAB WAIFU ──────────────
WAIFU_HUSBANDO_BOT_IDS = [6195436879, 6546492683, 7438162678]  # ✅ Slave bot added at end
WAIFU_HUSBANDO_BOT_USERNAMES = ["Waifu_Grabber_Bot", "Husbando_Grabber_Bot", "Slave_waifu_bot"]
...
@client.on(events.NewMessage(pattern=r"^\.refresh$", from_users=OWNER_ID))
async def refresh_data(event):
    try:
        global GALIS, TRIGGERS, profile_pic
        with open("gali.txt", "r", encoding="utf-8") as f:
            GALIS = [line.strip() for line in f if line.strip()]
        with open("triggers.txt", "r", encoding="utf-8") as f:
            TRIGGERS = [line.strip() for line in f if line.strip()]
        with open("pfp.jpg", "rb") as f:
            profile_pic = f.read()
        await event.reply("♻️ Files refreshed successfully.")
        print("🔄 Refreshed: gali.txt, triggers.txt, pfp.jpg")
    except Exception as e:
        await event.reply(f"❌ Refresh failed: {e}")
